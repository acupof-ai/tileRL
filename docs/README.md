...
| [lessons/](lessons/) | Notes on driving Claude Code against tileRL's own server (the Messages shim, the rollout launcher's sandbox) and cross-cutting test/audit guidance such as why a name-based dead-code scan false-positives on framework-reflected symbols. (historical notes kept as CHANGELOG evidence) |
