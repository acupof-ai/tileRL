...
`docs/bench-inventory.md` 已判定：Keep 33，Delete 22（10 bench 含 `bench_smoke.py` 移 CI、4 ab、4 sweep/matrix、4 launcher）。判定已记录，这是执行不是评审。22 个已在 #378/#379 删完；inventory 的分诊表已消费，只留 registry 装不下的残渣（conditional verdicts、sm70 系列、SSD 支持）。
