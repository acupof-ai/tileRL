...
| M3 | **KV pool allocates 64 layer planes; only 16 are ever touched** — `kv_cache.py:98-100` shapes on `num_layers`, indexed by the *global* layer index, while only `cfg.is_full_attn(i)` layers write | **0.75 GiB** at serve default, **3.00 GiB** at the bench's 1024 blocks | Executed with full-attn at global index 3: planes 0/1/2 `k_nnz=0 v_nnz=0`, plane 3 `928`. 3 of every 4 MiB per block is permanently zero. Zero runtime signature. The sharp framing is the serve default: `cli.py:64` gives 256 blocks = 4096 tokens of pool against `max_total_tokens=8192` — the pool is already binding at half the configured context, and 75% of it is zeros. A dense pool makes the same 1 GiB hold 16384 tokens. arle's function is literally named `alloc_full_attn_kv_pool` (`executor/qwen35.rs:957-983`). |

**Fixed resident recoverable: 5.49 GiB** at serve defaults (25.04 → 19.55 GiB), 7.7 GiB at bench settings, plus removing an unbounded leak that ends any long run. Note the tempting wrong fix: making the state pool f32 to kill P2's casts *costs* +576 MiB and only nets 3.0 MiB/layer/tick — the right fix is reading the bf16 slot in place inside `gdn_decode_fused` (3.15 MB/layer, **half** arle's f32 traffic), with a training carve-out because `linear_attn_chunk`'s recorded `state` argument must stay private under a Tape.
