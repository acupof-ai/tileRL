...
| entry | path | fix |
|---|---|---|
| [sm70 paged_attention launch hangs forever after SSE churn, wedging the whole engine](errors/2026-09-15-sm70-paged-attention-launch-hang-wedges-engine.md) | device forward on the engine loop / `engine._lock`; `/health` stale snapshot | after ~16 late-SSE disconnects at ~114 MiB free, one `paged_attention_split` launch (B=3 S=512, NB=2214) never returns: loop R-state holds `_lock`, GPU 0%, no Xid, submit/cancel block, /health falsely 200. **B dim and NB=2214 ruled out by `scripts/repro_sm70_split_b3_hang.py` (all shapes return in ms).** Unreproduced; leading hypothesis is edge-memory caching-allocator host-side block, not the kernel. Fix is resilience, not cancelling the C thread: (1) /health step last-progress timestamp → 503 when stuck; (2) forward wall watchdog → controlled process exit for supervisor restart; (3) larger VRAM admission headroom |
