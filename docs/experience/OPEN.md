...
| [prefetch abandoned at deadline: GIL contention](errors/2026-09-10-prefetch-deadline-gil-contention.md) | `src/tilerl/engine.py` `_build_plan` deadline; `kv_cache.py` `KvTier._fetch_loop` | Prefetch triggered (`ssd_prefetches=1`) but never completes (`ssd_fetches_ready=0, ssd_fetch_drops=1`): the 75 ms deadline (192 tokens ÷ 2558.6 tokens/s seed rate) expires before the fetch finishes. `torch.load` itself is 0.4 ms, but GIL contention with the main thread's `step()` loop makes it 71.7 ms in the reader thread — 175x slower. The mechanism only works where it is useless: faster cards get shorter deadlines and the same contention. Fix: yield the GIL in the step loop (`time.sleep(0)`), bringing the fetch to 0.5 ms. Measured 2026-09-10, H20 card 3 |
| GDN state differs when prefix-hit and fresh-prefill use different chunk sizes | `src/tilerl/engine.py` `_build_plan` first-chunk cut; GDN parallel scan | When a prefix hit restores at a boundary the warm-up reached with chunk size A and the fresh prefill reaches with chunk size B ≠ A, the GDN states differ by ~3e-4 (2.5% relative on near-zero elements). Source: the parallel scan rounds per chunk length. **No established upper bound, no test watching it.** Measured 2026-09-10, H20 card 1, see [errors/2026-09-10-gdn-state-chunk-size-rounding.md](errors/2026-09-10-gdn-state-chunk-size-rounding.md). The fix that closes it: a cross-config test that measures the bound and asserts against it — separate from #438 |
