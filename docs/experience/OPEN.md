...
| entry | path | fix |
|---|---|---|
| [256k sparse prefill host-OOMs on the SSD spill path](errors/2026-09-12-sparse-256k-spill-host-rss-oom.md) | `src/tilerl/kv_cache.py` `ColdSsdFile.write` / `HostKvPages._evict_to_ssd` (spill, #550) | sparse 27B 256k prefill with the mmap spill grows **anonymous python RSS ~22 MiB/s to VmRSS 30.7 GiB** and host-OOMs (rc=137) at 16–19 min on both 10/8 and 6/12 host/SSD splits; the 20 GiB no-SSD run completed (47.4 min). Page cache ruled out (`Cached` 3.98 GiB, `Dirty` 0 at OOM) — a host copy/pinned staging per spilled page is retained past the host budget. Retaining site unproven; first fix deliverable is the per-pid VmRSS + tier-counter curve. No 256k rerun until fixed |
| [under spec decode a sparse prefix hit cannot serve a follower](errors/2026-09-12-spec-follower-cannot-adopt-a-prefix.md) | `src/tilerl/engine.py` `_admit` prefix lookup / `spec.py` `DraftHead.forward` | #530 makes it return-miss (correctness fixed): with a draft attached, no prefix adoption, `matched=0` even on a dense hit; follower tokens bit-equal to cold. Under production defaults (sparse + spec) the prefix cache publishes but never serves — TTFT save dead on the spec path. Warm path needs trunk hidden at every adopted position: rerun the trunk forward the adoption skips, or store hiddens in the snapshot |
