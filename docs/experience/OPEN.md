...
| entry | path | fix |
|---|---|---|
| [sm90 sparse graph capture gives no speedup — finalize reads selected pages back to host every tick](errors/2026-09-16-sm90-sparse-graph-no-speedup-finalize-d2h.md) | `src/tilerl/sparse_engine.py:754-756` `selected_pages().tolist()`, `:816` `selected().tolist()`; called from `sparse_runtime.py:552` finalize | H20 sm90 sparse graph-on vs eager ≈1.0x (32k 21 vs 20 tok/s). nsys 32k: the decode forward IS captured (11 `cudaGraphLaunch`/12 forwards; 1-in-8 refresh ticks eager), but each forward does **126 D2H + 118 `cudaStreamSynchronize`** in post-replay finalize (79.8% of host API wall), GPU busy only **12.4%** (kernels 5.33 ms of 42.9 ms). dense graph control: 2.3 D2H/forward, 25 ms, near GPU-bound. Not launch gaps, not graph coverage — the host pin/evict page set round-trips device page picks; both arms wait on it. Fix: device-resident pin/evict table, no `.tolist()` in finalize (or batch readback at the refresh tick); likely same root on sm70 eager. Queued for fixkv after #667. |
