...
| entry | path | fix |
|---|---|---|
| [a full-page prefix hit feeds the last page to the GDN state twice](errors/2026-09-24-prefix-hit-feeds-the-last-page-twice.md) | `_admit` full-page-aligned prefix-hit branch (`src/tilerl/engine.py`) + the re-forward in `_finish_prefills` | the adopted snapshot is the state AFTER the last page and the re-forward consumes that page again. Measured CPU tiny: restored state is **bit-identical** to a prefix-MISS oracle and the re-forward then moves it by **26.875** against `max|state| 28.125` (**0.956** relative, vs the 1.2e-2 bound the same path carries). First token unchanged, which is why the existing test stayed green. **Fix LANDED**: write the restored state back before sampling/publishing; gate red without it |
| [sparse device-select attends the wrong page when candidates have holes](errors/2026-09-24-sparse-device-select-reads-the-wrong-page.md) | `SparseForward._select_device` `s_l2p.gather(1, chosen)` (`src/tilerl/sparse_engine.py`) | `s_l2p` is indexed by **candidate position** (`fill()` writes `s_l2p[bi,:nc] = l2p[cand]`) but gathered by **logical page**; `cand` is a filtered range, so a hole shifts every later page by one. Measured: hole at page 0 -> logical 0 returns block **-1** where 172 is correct, and the wrong slot is usually a VALID block so the cold mask never fires — silent wrong-page attention inside the captured graph. Invisible at the shipped default because `cand == range(n)` there. **Fix LANDED**: gather with the candidate position; gate red without it |
| [a new request inherits the previous requests' sparse refresh phase](errors/2026-09-24-refresh-phase-inherited-across-requests.md) | `SparseRuntime.ticks_since_refresh` (`src/tilerl/sparse_runtime.py:120` init, `:301`/`:839` advance, `:307` reset) | engine-wide and never reset at admission, so a request's eager refresh ticks — and its served tokens — depend on the decode ticks the traffic before it consumed. Measured V100 sm70 min0, eight identical 32k requests: entry phases 3, 6, 0, 0, 0, 0, 0, 0 against three distinct output classes one-to-one in order (879 / 401 / 286 chars, last five byte-identical); a second run of the same shape gave a period-4 cycle instead, predicted by the same rule before it ran. **Finding 1 fixed by #821**: reset at admission when no other decode row is in flight; gate `tests/test_sparse_refresh_phase.py`, negative control reading the inherited 6. **This row tracks the entry's SECOND finding and stays until that is fixed**: the `--slots 1` degeneracy at 32k (256 repeated tokens). The entry therefore does NOT close with #821 |
| [b1024/ORDER B +38 ms outside-graph, reproducible](errors/2026-09-23-b1024-order-b-outside-graph.md) | `engine.step()` host work outside the graph envelope, at cmax bucket 1024 in run order B (`src/tilerl/engine.py` `_build_plan` / `_build_stats` / `_hybrid_charge`) | phase window `1d106e6a`: 16 of 18 cells agree between orders, and the two that do not are both b1024/ORDER B, in two different arms, with `step − graph` of **41.909** and **41.911 ms** — three-decimal agreement across arms, so deterministic and not jitter. Same pairing misbehaved in two earlier windows (parity: control-B b1024 W=1 graph 250.8 vs 86.8/85.1; patch-B b1024 W=2 eager 974.7 vs 237.7). Not the allocator: per-order allocator event counts are identical (12 `alloc_reclaim`, 53 `dev_malloc`, 1732 `gpu_drain`; min `free=2884 MiB` both). The extra time lands in `p_rows`/`p0_fill`/`p4_verify` together (3–5x), not one phase. **Mechanism undetermined — no fix named yet.** Re-measure both orders before quoting any b1024 latency |
| [shared prefix SSD spill ignores --cold-ssd-bytes and the extent file never shrinks](errors/2026-09-19-sm70-32k-decode-physical-wall-and-ssd-close-tail.md) | `ColdSsdFile` shared `.prefix.bin` spill (`src/tilerl/kv_tiers.py`) | the shared prefix cache publishes pages nothing serves; pre-fix the spill grew to 13.6 GiB logical / 25 GiB physical vs an 8 GiB cap. **Cap + trailing-extent truncation LANDED env-gated/default-off (#740 `TILERL_COLD_PREFIX_SSD_CAP=1`); device-read 2026-09-19 proves the cap plateau (physical file pinned 8192 MiB under load, 0 drops) but post-release reclaim was NOT sampled — pending-next-window; the follower byte-identical/prefix_hits smoke was client-terminal only. NEW TAIL: at a full cap the evict/reload churn costs one warm rep 9.25→5.84 effective tok/s (tick p90 738 ms, pub_share_hold max 5.9 s, close max 7.65 s) — bounding a publish-but-rarely-served cache protects disk but not latency. Remaining: next-window post-release `du` evidence with spill retained and mid-file hole-punch (`FALLOC_FL_PUNCH_HOLE`/`F_PUNCHHOLE`). **The default-flip question is closed 2026-09-21:** close-side publishing is deleted wholesale (page publishes once on pool exit), so there is no "close bytes off-lock" to wait on — see errors/2026-09-21-optimizing-at-the-wrong-layer-close-scheduling.md.** |
