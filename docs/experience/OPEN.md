...
| entry | path | fix |
|---|---|---|
| [V100 sparse 256k prefill is SIGKILL under a 10 GiB host tier](errors/2026-09-13-v100-256k-sparse-prefill-host-oom.md) | V100/sm70 f16 cold pool; `src/tilerl/kv_cache.py` `HostKvPages` pin/demote; `build_engine` `--kv-cold-bytes` | k=128 B=1: **64k serves** (prefill 343 s, decode 3.44 tok/s, RSS 27.9/31 GiB), **256k SIGKILL in prefill with cold_ssd=0 and no spill file created**. Mechanism untested (cold-pool bytes at the kill missing): hypothesis = own-span pages pin into the 10 GiB host tier faster than prefill demotes. Options: bigger host budget/RAM, prefill-time eager demotion, or cap V100 sparse at 64k |
| [an sm90 B=8 spec wave is not reproducible across identical cold waves](errors/2026-09-13-sm90-b8-spec-wave-not-reproducible.md) | owner cc; sm90 B>1 sparse spec tick (packed prefill/verify or batched draft) | two identical fresh COLD B=8 waves agree only **3/8** (rows 0/1/5/6/7 diverge at tokens 27/30/31/5/30) under the ACTIVE #563 unfused guard; B=1 sequential warm-vs-cold is 8/8 exact (logits max_abs 0.0) and CPU tiny B=8 is 8/8. Not the fused-routing bug (#567, guarded off here): reproducibility defect, sparse-vs-sparse, needs a served-shape per-row first-logit dump |
| [served sparse decode runs eager, not the sparse graph](errors/2026-09-13-served-sparse-decode-eager-not-graph.md) | owner 52/5f; `src/tilerl/engine.py` dispatch + `build_engine` default | two gaps kept every served sparse decode tick eager (~88 ms/tick vs ~13 ms dense captured, card-3 B=1 k=128): the dense `_run_decode_graph` shadowed the sparse graph (captured `sparse=None`, buckets=0 on card-2), and production never set `sparse_device_select`. #557 fixes both and pins them with CPU forced-graph gates; **the card-2 PATH counters on the served default (sparse_graph ticks, buckets>0 across the 8-tick refresh, cap-vs-eager token equality, ms/tick) are pending the next H20 window** |
...
| [prefetch deadline too thin on CPU: fetch drops under load](errors/2026-09-10-prefetch-deadline-gil-contention.md) | `src/tilerl/engine.py` `step` spin; `kv_cache.py` `KvTier.any_fetching` | **Fixed 2026-09-10** by the spin-until-ready loop (72d83303, [wins entry](wins/2026-09-10-spin-until-ready-fixes-cpu-prefetch-flake.md)): the e2e flake went 3/16 → 0/20. **Remainder measured 2026-09-10** ([errors entry](errors/2026-09-10-the-27b-snapshot-fetch-exceeds-the-prefetch-deadline.md)): the spin hits the 50 ms bound on 3/3 spin ticks, and the 157.6 MiB fetch (117 ms) exceeds the 75 ms deadline, so it drops — the prefetch is structurally broken on 27B sm90. **Fixed 2026-09-10** by never discarding an in-flight fetch — the deadline gates the wait, not the read ([wins entry](wins/2026-09-10-never-discard-an-inflight-fetch.md)): the deadline-missed fetch parks with 0 drops and serves the next same-prefix request without a new read; the spin is narrowed to a waiting row's own fetch, closing the global-`any_fetching()` cost |
