...
| entry | path | fix |
|---|---|---|
| [sm70 32k per-request cold SSD close blocks the close path ~3.1 s](errors/2026-09-19-sm70-32k-decode-physical-wall-and-ssd-close-tail.md) | `HostKvPages` cold shared-page transfer at request close (`release_close_request` / `ssd_mmap` / `pub_cold_transfer`, `src/tilerl/kv_tiers.py`, `src/tilerl/sparse_engine.py`) | V100 1 GiB-RAM/8 GiB-SSD: once per request the close paid ssd_mmap ~1.83–1.86 s + pub_cold_transfer ~1.73–1.78 s. **#741 (batch D2H, env-gated) cut per-page syncs, close 2940→2321 ms. #743 bg publisher (`TILERL_CLOSE_BG_PUBLISH=1`) device-read 2026-09-20 (wins/2026-09-20-background-close-publish.md): at a fitting queue pub_cold_transfer 1783→9 ms and the spill grows after the request returns (bytes leave the step thread), cancel frees in 0.4–0.8 s off-lock, steady decode holds — but two gaps keep both gates default-OFF:** (a) **default `TILERL_CLOSE_BG_DEPTH=512` overflows a 32k close (~3100–4700 jobs) → 34.5% inline fallback, close not improved; fix = derive depth from max close burst (small PR, 8192 fit with 0 inline);** (b) **the worker's private-spill lift (`share_hold_kv` off sparse_cold.bin) still charges ssd_mmap ~3.3 s into release_close_request; bg2 close was 1020/6478 ms (n=2, thin+long-tail, not sub-second). Device-busy vs host-blocked (_tlock contention) is UNDECIDED — fwd_gpu is a no-sync event span that inflates on a blocking wait, do not call it GPU-busy; static read assigned to fixkv, then a non-blocking event-query + worker-mmap probe in the next window.** A backlogged follower also waits ~4.9 s off-lock (no tick lock held, but latency exists). Default flip waits on both |
