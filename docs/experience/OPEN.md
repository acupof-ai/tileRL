...
| [think-off single request runs at 48.1 tok/s, below the 50 target](errors/2026-09-14-think-off-single-request-below-target.md) | unattributed; sm70 serve decode | think-off solo 48.1 tok/s; one bimodal run at 46.4 split into 39.5 vs 35.6 ms/fwd with no config change. Fix: phase-attributed per-forward timing, A-vs-A in one process, then a second-card placement control |
