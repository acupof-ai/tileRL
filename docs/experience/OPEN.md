...
| [four concurrent dense+d1 rows 500 at a decode block boundary](errors/2026-09-15-dense-d1-concurrent-block-boundary-assert.md) | owner fixkv; `src/tilerl/spec.py` `DraftHead.step` block planning + dense decode growth in `engine.py` | V100 sm70 hybrid serve, the 4x2k warmup arm 500s ~1/3 with `AssertionError: draft would write position 928 but the row owns 58 blocks x 16 = 928 (seq_len=929, draft_pos=926)`; one tick fails every running request. Pre-existing on 87869c62/3026d507/ff3eaa4a — not #616 (byte-identical graph path) nor #615 (boot-gated). Single request unaffected. Hypothesis (unverified): multi-row trunk-vs-draft block accounting one block short at the crossing. Fix queued after step 11; needs a deterministic tiny CPU 4-row boundary gate, not another ~1/3 card warmup |
