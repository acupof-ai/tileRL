...
| [a full cold tier makes sm70 sparse 32k decode long-tail](errors/2026-09-17-cold-tier-full-finalize-relocation-long-tail.md) | KV cold tier at 8 GB / device_free ~200 MB; `sparse_finalize` page moves + the per-request close-time lock stall | warm 32k serves 7.86 tok/s with an empty cold tier but 2.69–6.56 full; steady tick p50 healthy ~185 ms but the long tail and a deterministic per-request 1.8–8.9 s close stall (in the `sample` bucket, under `engine._lock`) dominate. **2026-09-18 four-arm 0/384/768/0 verdict: `--device-headroom-mib` is a no-op** — all arms size the same 2213-block pool (-0), steady p50 184–186 ms and fill finalize are the same distribution; the build solver never prices the runtime host cold tier. Host O(n) LRU/spill cost fixed in #735 (device delta pending). Remaining fix: attribute the close lock with the #732 five sub-segments on the next V100 window (byte volume cannot explain it; per-page sync/pinned churn leads), then batch/lock-move per that read |
