...
| entry | path | fix |
|---|---|---|
| [sm90 fused attn_prep corrupts sparse K/V on B>1 packed prefill](errors/2026-09-12-sm90-fused-attn-prep-sparse-packed-prefill.md) | `packages/tilerl-kernels/.../kernels_mma.py` `make_attn_prep`/`make_attn_prep_fp8` | #563 guards it (sparse forces the unfused writer, prefill+decode). Matched B=8 g0 dense-vs-sparse max_abs 8-11 fused, 0 unfused; B=1/sm70 exact. **Still open even guarded:** sparse k=128 spec-off acc 0.7986@139/400 vs 0.915 dense at full page coverage — a further sm90 sparse defect (decode path or another prefill cell); default stays sparse-OFF. Fix = fused twin correct under B>1 ragged rows + the residual gap closed, served-dims sm90 gate |
