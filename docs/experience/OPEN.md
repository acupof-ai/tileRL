...
| entry | path | fix |
|---|---|---|
| [sm70 32k per-request cold SSD close blocks the close path ~3.1 s](errors/2026-09-19-sm70-32k-decode-physical-wall-and-ssd-close-tail.md) | `HostKvPages` cold shared-page transfer at request close (`release_close_request` / `ssd_mmap` / `pub_cold_transfer`, `src/tilerl/kv_tiers.py`, `src/tilerl/sparse_engine.py`) | V100 1 GiB-RAM/8 GiB-SSD: once per request the close paid ssd_mmap ~1.83–1.86 s + pub_cold_transfer ~1.73–1.78 s. **PR1 LANDED (#741 `TILERL_CLOSE_BATCH_D2H=1`; close median 2940→2321 ms −21%, steady unchanged) — per-page D2H syncs only. PR2 LANDED CPU-gated 2026-09-20 (`TILERL_CLOSE_BG_PUBLISH=1`, wins/2026-09-20-background-close-publish.md): a bounded single-consumer daemon moves the host/SSD private→shared transfer off the close segment, followers block on per-key futures with a timeout that degrades to a miss, shutdown joins; 14 new CPU gates, full suite 1025 passed. Remaining (perf1, next V100 window): device re-measure with the #732 sub-segments proving ssd_mmap + pub_cold_transfer leave the close segment while steady decode (~166 ms/tick, ~9.4 tok/s) holds; both gates stay opt-in until that number lands, then consider the default flip** |
