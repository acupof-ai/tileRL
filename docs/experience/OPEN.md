...
| [prefetch deadline too thin on CPU: fetch drops under load](errors/2026-09-10-prefetch-deadline-gil-contention.md) | `src/tilerl/engine.py` `_build_plan` deadline; `kv_cache.py` `KvTier._fetch_loop` | The GIL yield fix landed (1dc563fb), but on CPU the deadline is still thin: 2.56 s at 192 tokens vs a 1.2–2.4 s GIL-starved fetch on a loaded box. The e2e test flakes 3/16 on a dev machine (clean main, pre-existing). The test's clock was hardened (`_drain_clock`) but the engine's deadline was not — second instance of the same shape. Length sweep: margin grows 1.05x → 1.55x then plateaus (fetch is partially byte-bound), so lengthening alone does not fix it. N-sweep: N≥10 yields/tick brings the fetch to the 2 ms uncontended floor. Fix direction (approved): spin until the fetch is ready with a wall-clock bound, not a fixed N — the reader misses most single-yield windows (it is mid-I/O, not GIL-blocked, when the yield fires), so N=1 catches a window only ~once per 14 ticks; a spin catches one within ~1 tick. The bound is a safety valve for a stuck reader, not a tuned parameter |
