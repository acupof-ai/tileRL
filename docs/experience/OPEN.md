...
| entry | path | fix |
|---|---|---|
| [sm70 serve wedges under a simultaneous-SSE-hangup cancel storm (GIL spin), not a CUDA launch](errors/2026-09-15-sm70-paged-attention-launch-hang-wedges-engine.md) | `src/tilerl/server.py` SSE final drain; `/health` stale snapshot | **Root-caused 2026-09-16; the title's paged_attention launch hang was a misread.** `stream_or_cancel`'s finally awaited an in-flight `to_thread(next)` inside the SSE task's own latched anyio cancel scope; `await shield(worker); continue` re-raised on every checkpoint with no real waiter, so ~8 simultaneous hangups busy-spun the event loop and the engine `_loop` thread parked in `PyEval_RestoreThread` (gdb: not CUDA) — the apparent stuck leaf moved between ops. The VRAM/allocator hypothesis was measured and refuted (#654 trim held no free; #655 fraction saw zero fence hits). Fixed by #658: the sync body is finalized in a detached task (module strong-ref set) that cancels the idempotent row, bounded-awaits the worker, and closes on a worker thread; skipped closes are warned; graceful shutdown joins drains via the app lifespan. Gates: deterministic structural AST pin + real-uvicorn 8-socket storm (both red on parent), 110 tests. **Line stays only until the V100 device confirmation passes** (ops: 20/20 SSE overload storm, pid/boot stable, 128k sparse) |
...
| [sm70 sparse decode graph + MTP d1 corrupts multi-token output](errors/2026-09-14-sm70-sparse-graph-mtp-corrupts-output.md) | sparse lazy graph capture keyed on `(B,W,cmax,own_w)` | sparse+d1+`--decode-graph` deterministically corrupts multi-token decode (1-token exact) and hit illegal access at B=4; eager and dense+graph exact. H1 (warmup scribbled live block 0) disproved on device after #585; H2 (capture at a cmax-bucket transition) untested. Hybrid #586 works around it by forcing sparse ticks eager. Fix: force a capture at a bucket transition against the pad frame, compare first-logit/`win_parity` vs eager |
