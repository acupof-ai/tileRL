...
divide 128 — the smallest of them.
...
gradient itself, but only on sm90 with the lever set: **the gate that would have
caught this exists and has never run in the configuration that breaks.**
CI is CPU-only, and the CPU and Metal cells use
...
from one name. Same shape as the two other defects found the same day: a loop
bound that floor-divides to zero and hands back its untouched buffer. When a
review predicts one, run the failing configuration before fixing it.
