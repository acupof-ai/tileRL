# The 112 casts do not hide behind their GEMVs, and 0.349 ms/token is not worth removing — sm70, 2026-09-06
...
that consumes it, and a DRAM-bound GEMV has idle SMs a cast might occupy.
...
- **Dense f16 matmul, not the fp4 kernel.** The fp4 GEMV needs quantized weights from the
  checkpoint, which is the capacity problem. What carries over is DRAM-boundness at M=1
  (~2 FLOP/byte), the property that decides whether a launch-bound cast can hide. The fp4
  kernel's own occupancy is not measured. If it were *more* occupied than the dense GEMV, the
  cast would hide even less, so the direction of this limitation does not threaten the reject.
...
|---|---|---|---|---|---|---|---|
| 2026-09-06 | 153f067 | V100 32GB | cuda sm70 | f16 dense, real widths | 112 casts, M=1 | marginal µs/cast | **3.12** |
| 2026-09-06 | 153f067 | V100 32GB | cuda sm70 | f16 dense, real widths | 112 casts, M=1 | ms/token | **0.349** |
| 2026-09-06 | 153f067 | V100 32GB | cuda sm70 | f16 dense, real widths | empty graph | replay µs | 13.1 |
| 2026-09-06 | 153f067 | V100 32GB | cuda sm70 | f16 dense, real widths | 112 pairs | fraction hidden | **unresolvable** (5.6% vs 18.4% spread) |

No runtime change — a probe, this entry, and one OPEN.md row removed.
