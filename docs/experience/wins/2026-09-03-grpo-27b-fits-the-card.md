...
1. **Recompute the MLP block instead of storing it.** `autograd.checkpoint`
   ...
   backward; `Model._mlp` wraps `_mlp_body` in it. The MLP is the only pure
   block in a layer — GDN advances the state pool, so it cannot be replayed.
...
Measured on one idle H20, one process per arm, same session. 27B, group 8,
T=275, LoRA rank 16.
...
Memory only. A 100-step run of the recipe was started on the same card and had
not printed step 1 after 66 minutes. Card memory sat flat at 41.3 GiB and
utilisation at 29-64%; TileLang compiled 702 kernels (120 of them
`rmsnorm_bwd_x`) and then stopped — the count was unchanged over three minutes,
so the cold cache is paid and the process is in real work. Nothing had ever got
past the OOM to walk the 27B training backward, so those 702 are first-time
compiles, not a recompile loop.

Not a memory failure. But the step's wall clock is now the open question, and
the split between rollout and backward is not observable from outside: the
caching allocator's reserved pool does not shrink when the rollout ends, so
`nvidia-smi` reads the same 41.3 GiB either way. `secs_per_step_median` and
whether reward rises stay unmeasured. The next run should log per-phase
timings.
...
Re-measured after rebasing onto main's GDN prep/post kernels: 33.53 / 10.30 at
`micro=1`, 81.24 / 58.01 at `micro=0`. Unmoved.

Raw artifacts: `scripts/probe_tape_mem.py` (the four-arm harness),
`/work/memab.log`, `/work/memab2.log`, `/work/remeasure.log`,
`/work/grpo100d.log` on the pod.
