...
here, and **1337 ms durable on the pod's `/work`** measured 2026-09-06
(`errors/2026-09-06-ssd-save-ms-is-page-cache-time.md`; the 641.8 first published was this
Mac's, and page-cache time at that), and the whole
...
prefill tick would cost more than the tier saves — **and on the pod's `/work` the durable save
is 1337 ms, not ~100** (`errors/2026-09-06-ssd-save-ms-is-page-cache-time.md`), so the
durability window is 13x wider than this section assumed and the off-tick decision is more
right, not less. So an entry published within the last
