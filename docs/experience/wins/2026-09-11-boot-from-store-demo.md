...
first-token logits. The admit now re-forwards the last loaded page once — its
K/V are overwritten with the identical values and the tail position's logits
appear — costing the same one page as any `<16`-token tail
