...
class Backend:
    ...
    def _wy_eligible(self, t: int, kw: dict, chunkable: bool) -> bool:
        """Does this call go through the WY kernels? ONE predicate, because the forward and
        the backward's recompute must agree: if the backward recomputes with the WY kernels
        at C=64 while the forward took the serial path, the adjoint belongs to a chunking
        the forward never ran. Re-deriving this at a second call site is the seam bug that
    ...
    def _gdn_wy_core(self, q, k, v, g, beta, state, chunk: int = _WY_CHUNK):
        ...
        Returns (out, new_state, saved) where saved carries the stage outputs the
        backward's adjoints read as inputs — the forward computed them and used to
        drop them, so a backward that wants them had to recompute the whole chunk
        loop in python.
        """
