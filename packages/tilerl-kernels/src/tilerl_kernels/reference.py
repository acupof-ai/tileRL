...
def _scan_gather_pairs(a, b, pg, rank: int, world: int, chunk_ids=None):
    ...
    The reverse scan must multiply the same pairs in the same order the forward
    folded them, so both share this. Returns ``(ca, cb, ids, want, entries)``:
    ``ca[k]/cb[k]`` is the pair at sequence position k (chunk id ``ids[k]``),
    ``want`` maps this rank's chunk id -> its local stacking index.
    """
    ...
    return ca, cb, seq_ids, want, entries
...
def affine_prefix_scan_bwd(goa, gob, a, b, pg, rank: int, world: int, chunk_ids=None):
    ...
    """
    ca, cb, seq_ids, want, _ = _scan_gather_pairs(a, b, pg, rank, world, chunk_ids)
    ...
    gpa, gpb, _, _, _ = _scan_gather_pairs(goa, gob, pg, rank, world, chunk_ids)
    kl = ca.shape[0]
