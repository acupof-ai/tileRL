#!/usr/bin/env bash
# Dev-only: launch the Qwen3.8-27B baseline bench detached (the load is a
# silent CPU-bound 10+ min, longer than tn exec's 5-min no-output timeout).
# Logs to /work/tilerl_baseline.log; stamps /work/tilerl_baseline.done.
set -e
