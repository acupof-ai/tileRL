...
def _served_bytes(t: torch.Tensor, quantized: bool) -> tuple[int, str]:
    """Bytes one forward reads for this tensor, and the kind it was counted as."""
    n = t.numel()
    if quantized:
        return n * _FP4_BITS // 8 + (n // _SCALE_BLOCK) * 4, "fp4+scale"
...
def main() -> int:
    ...
    # block size or an extra oscale plane would move every number below.
    ...
    model, _ = _served_bytes(probe, quantized=True)
