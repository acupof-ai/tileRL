...
def teacher_forced_logits(model, backend, cfg, eng, full_ids, wanted):
    ...
    wanted positions (no sampling). Used to teacher-force a continuation and
    score it under another engine."""
    cap, restore = attach_capture(model, cfg, backend, set(wanted))
    ...
    for _ in range(16):
        if rid in eng.poll():
            break
        eng.step()
    lg = {p: cap[int(p)] for p in wanted}
    restore()
    return lg
...
def nll_sweep(model, backend, cfg, spans, ks, n, num_blocks, out, cold_format):
    ...
    under dense. Returns mean NLL(S|dense), NLL(D|dense), top5(S under dense)."""
    dense = make_engine(model, backend, spans[0].shape[0] + n + 64, 0, num_blocks,
                        cold_format)
    agg = {k: {"nll_sparse": 0.0, "nll_dense": 0.0, "sparse_in_top5": 0.0}
           for k in ks}
    for si, prompt in enumerate(spans):
        dseq = generate_greedy(dense, model, prompt, n)
        nll_d, _ = nll_under(dense, model, backend, cfg, prompt, dseq, n)
        for k in ks:
            sparse = make_engine(model, backend, len(prompt) + n + 64, k,
                                 num_blocks, cold_format)
            sseq = generate_greedy(sparse, model, prompt, n)
            nll_s, in5 = nll_under(dense, model, backend, cfg, prompt, sseq, n)
            sparse.shutdown()
            if backend.device.type == "cuda":
            ...
            print(f"span {si} k={k} nll_sparse={nll_s:.4f} nll_dense={nll_d:.4f} "
                  f"top5={in5:.3f}", flush=True)
    dense.shutdown()
    ns = len(spans)
    result = {f"sparse_k={k}": {key: round(v / ns, 5) for key, v in row.items()}
              for k, row in agg.items()}
    for k in ks:
        result[f"sparse_k={k}"]["nll_gap_sparse_minus_dense"] = round(
            result[f"sparse_k={k}"]["nll_sparse"]
            - result[f"sparse_k={k}"]["nll_dense"], 5)
    result["_n_spans"] = ns
    result["_n_greedy"] = n
    with open(out, "w") as fh:
        json.dump(result, fh, indent=2)
...
def main27b(args):
    ...
    if args.nll:
        ...
        nll_sweep(model, backend, model.cfg, spans, ks, 64, num_blocks=0,
                  out=args.out, cold_format=args.cold_format)
        return
