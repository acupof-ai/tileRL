...
scale = D**-0.5
ref = backend._kernel("paged_attention")(q, k, v, bt, lens, torch.ones(B, dtype=torch.int32, device="cuda"), scale, BS, 16, 128)[:, :1].float()
out = backend._paged_attention_decode(q, k, v, bt, lens, scale).float()
print(f"len {n}: relerr {bk.relerr(out, ref):.3e}")
ms_ref = bk.timeit(lambda: backend._kernel("paged_attention")(q, k, v, bt, lens, torch.ones(B, dtype=torch.int32, device="cuda"), scale, BS, 16, 128), 20)
ms_new = bk.timeit(lambda: backend._paged_attention_decode(q, k, v, bt, lens, scale), 20)
