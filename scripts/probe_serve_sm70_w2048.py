...
MIN_NEW_BUCKET = 4096
# Full-sequence quality line (#805 cutover): a graph-vs-eager divergence is
# tolerated only from this generated-token position onward; an earlier one is a
# real gate red (rc1). Same 128 = 16 refresh-periods line as the v2 plan.
FULLSEQ_DIV_MIN = 128


def fullseq_verdict(graph, ref):
    """Pure full-sequence comparison of the graph arm against the forced-eager
    reference. Returns (status, first_diverge, red):
      status "identical" / "diverge"; first_diverge None or index; red True when
    the first divergence is BEFORE FULLSEQ_DIV_MIN (the gate rejects it).
    Unequal lengths must be rejected earlier by the caller (cannot align)."""
    fd = next((i for i, (a, b) in enumerate(zip(graph, ref)) if a != b), None)
    red = fd is not None and fd < FULLSEQ_DIV_MIN
    return ("identical" if fd is None else "diverge", fd, red)
...
def run_worker(arm, args):
    ...
    # The reference eager arm is measured too: the cutover window compares the
    # graph arm's warm throughput against the forced-eager arm in the SAME
    # window, so both must record a throughput block.
    measure = arm in ("graph_w2048", "baseline", "ref_eager_w2048") and not smoke
    eff_tokens = 0
    wall_ms = 0.0
    ...
    try:
        for pi, ids in enumerate(prompts):
            ...
            # Per-prompt FULL token sequence for EVERY arm (full-sequence
            # comparator / noise-floor input), independent of the 09-17 gate.
            # Namespaced by arm because all arms share one --per-prompt-dir
            # (a plain name would let later arms overwrite earlier arms).
            pp_dir = getattr(args, "per_prompt_dir", "")
            if pp_dir:
                with open(os.path.join(pp_dir, f"{arm}_{pi:03d}.json"), "w") as f:
                    json.dump({"output": r["output"]}, f)
                # The graph arm ALSO writes the un-namespaced name the existing
                # noise-floor comparator (compare_refresh_runs.py) reads.
                if arm == "graph_w2048":
                ...
                if floor_run:
                    ...
                    if len(ref) != len(r["output"]):
                        raise ProbeFail(
                            ...
                            f"mismatch", rc=14)
                    # FULL-SEQUENCE gate (#805 cutover): compare the graph arm's
                    # whole output to the forced-eager reference via the shared
                    # pure function (the negative control mutates this object).
                    _fs, fd, _red = fullseq_verdict(r["output"], ref)
                    row["fullseq"] = "identical" if fd is None else fd
                    if _red:
                        results["failures"].append(
                            f"prompt{pi} FULL-SEQUENCE first divergence {fd} < "
                            f"{FULLSEQ_DIV_MIN}: graph={r['output'][max(0,fd-3):fd+3]} "
                            f"ref={ref[max(0,fd-3):fd+3]}")
