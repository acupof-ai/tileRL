...
def calibration(rows: list[dict], device_name: str,
    ...
    pending-remote rather than dividing by a half-calibration). A physical ``uuid``
    picks that card's own floors when a uuid row exists. The tensor peak is bf16 where
    the arch has it and f16 on pre-Ampere cards (sm70 V100); the roofline needs one
    flop ceiling."""
