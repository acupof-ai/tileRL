...
def _device_free(args, backend) -> int:
    ...
    sys.exit(
        "error: --dry-run budget rows need --device-free BYTES off CUDA "
...
def _dry_run_checkpoint(args, backend) -> None:
    ...
    cfg = {
        "tiny": config_mod.tiny,
        "tiny-agent": lambda: config_mod.tiny(65536),
        "qwen38-27b": config_mod.qwen38_27b,
    }[args.model]()
    ...
    rows = plan(
        ...
        None,
        free_after_fixed,
        num_slots=state_slots,
        num_blocks=num_blocks,
        state_dtype=_torch.float32,
        kv_io=_torch.bfloat16,
        kv_fp8=kv_fp8,
        explicit_state_budget=args.state_bytes,
        dram_budget=args.dram_bytes,
        ckpt_faces=faces,
        sparse=_sparse_spec(args, cfg),
    )
    ...
    if args.json:
        ...
        print(
            f"tilerl serve --dry-run: model={cfg.name} checkpoint={args.checkpoint} "
...
def _apply_device_reserve(args, backend) -> None:
    ...
    print(
        f"serve: device memory fraction {frac:.5f} "
        f"(reserve {reserve / 2**20:.0f} MiB of {total / 2**30:.2f} GiB held)",
        flush=True,
    )
...
def cmd_serve(args: argparse.Namespace) -> None:
    ...
    if args.checkpoint:
        if not args.dry_run:
            sys.exit(
                "error: --checkpoint is a --dry-run header-only query; "
    ...
    if args.dry_run:
        ...
        if getattr(args, "sparse_k", 0) and getattr(args, "checkpoint", ""):
            ...
            sys.exit(
                "error: --sparse-k with a built engine prints the live sparse ledger; "
        ...
        if getattr(engine, "_sparse", None) is not None:
            ...
            draft_layers = (
                engine._draft.cfg.num_layers
                if getattr(engine._draft, "kv", None) is not None
                else 0
            )
            rows = plan(
                cfg,
        ...
        if args.json:
            ...
            print(
                f"tilerl serve --dry-run: model={cfg.name} target={backend.target} "
        ...
        if getattr(args, "record_residency", False):
            ...
            rid = append_residency(
                residency_row(
                    name, card, peak, static, transient, backend.arch, model=cfg.name, uuid=uuid
                )
            )
            print(
                f"appended device_resident_bytes peak {peak:,} = static {static:,} + "
    ...
    print(
        f"tilerl serve: model={cfg.name} target={backend.target} "
...
def cmd_merge(args: argparse.Namespace) -> None:
    ...
    m = new_manifest(
        ...
        {
            "base": args.base,
            "specialists": list(args.specialists),
            "method": args.method,
            "commit": commit(),
        },
    )
    ...
    m["parents"] = [
        pid for pid in (find_run_for_artifact(runs_root(), p) for p in paths) if pid is not None
    ]
    ...
    if args.json:
        ...
        print(
            f"merged {len(args.specialists)} specialists ({args.method}) -> {args.out}  run {m['id']}"
        )
...
def cmd_ledger(args: argparse.Namespace) -> None:
    ...
    if args.devices:
        ...
        print(json.dumps(sections, indent=1) if args.json else _format_device_ledger(sections))
        return
    ...
    if args.time_to_score is not None:
        ...
        if args.json:
            print(
                json.dumps([{"id": m["id"], **(r or {"reached": None})} for m, r in rows], indent=1)
            )
            return
        for m, r in rows:
            if r is None:
                ...
                dip = (
                    ""
                    if r.get("held", True)
                    else f"  [fell below {args.time_to_score:.3g} again at step "
                    f"{r['dipped_at']}: a transient crossing, not arrival]"
                )
                print(
                    f"{m['id']}  score {args.time_to_score:.3g} reached at step "
                ...
                print(
                    f"{m['id']}  score {args.time_to_score:.3g} NOT reached in "
...
def _build_parser(recipe: str | None = None) -> argparse.ArgumentParser:
    ...
    p_serve.add_argument(
        "--json", action="store_true", help="with --dry-run, print the memory rows as JSON"
    )
    p_serve.add_argument(
        "--record-residency",
        action="store_true",
    ...
    p_serve.add_argument(
        "--draft",
        help="MTP/NextN head safetensors: speculative decode. For "
    ...
    p_serve.add_argument(
        ...
        default=12,
        metavar="N",
        help="token headroom buffered before paced SSE starts; must cover one "
        "refresh stall (12 tokens ~288 ms covers the ~259 ms R32 refresh).",
    )

    p_serve.add_argument(
        "--sparse-k",
        type=int,
        default=DEFAULT_SPARSE_K,
        metavar="PAGES",
    ...
    p_train = sub.add_parser(
        "train", help="SFT, --rl (GRPO) or --opd; --recipe for a gated flag set"
    )
    ...
    p_train.add_argument(
        "--indexer-di", type=int, default=128, help="indexer head dim (V4.1 releases 128)"
    )
    p_train.add_argument(
        "--recall-threshold",
        type=float,
    ...
    p_train.add_argument(
        "--data",
        help="JSONL {prompt, answer}: real prompts, exact-match "
        "reward on the last number (scripts/gsm8k_jsonl.py)",
    )
    p_train.add_argument(
        "--reward",
        choices=["number", "boxed"],
        default="number",
    ...
    p_train.add_argument(
        "--eval-gsm8k", help="JSONL {prompt, answer}: greedy exact-match accuracy before and after"
    )
    ...
    p_train.add_argument(
        "--json", action="store_true", help="print the run manifest as JSON instead of step lines"
    )
    p_train.add_argument("--lr", type=float, default=1e-3)
    p_train.add_argument(
        "--optim",
        choices=["adafactor", "iso"],
        default="adafactor",
    ...
    p_train.add_argument(
        "--depth",
        type=int,
        help="drafts per row per tick; default is the "
    ...
    p_train.add_argument(
        ...
        type=int,
        default=0,
        help="--dry-run: micro-batch rows B for the tape row (default: --micro, else --group)",
    )
    p_train.add_argument(
        "--train-seq-len",
        type=int,
    ...
    p_bench.add_argument("--card", type=int, default=None, help="physical GPU card for --calibrate")
    p_bench.add_argument(
        "--device-name",
        default=None,
    ...
    for v in ("table", "readme", "regress", "questions", "collectors"):
        p_bench.add_argument(
            f"--{v}", action="store_true", help=f"bench view: {v} from the bench store, no GPU"
        )
    p_bench.add_argument(
        "name",
        nargs="?",
    ...
    p_gen = sub.add_parser("generate", help="offline batch generation, one process per device")
    ...
    p_gen.add_argument("--max-batch", type=int, default=32, help="concurrent requests per device")
    ...
    p_merge.add_argument("--method", choices=["iso", "average", "ties", "dare"], default="iso")
    p_merge.add_argument(
        "--force",
        action="store_true",
