...
class Engine:
    ...
    def _memory_rows(self) -> list[dict]:
        ...
        # so pass 0 here (the fit already happened at build). The dry-run path calls plan
        # directly with the real number.
        ...
        # Aggregate the plan's sub-rows (state_slots is state + conv + spec + parity) into
        # one row per owner so derived and measured compare at the same granularity.
        agg: dict[str, dict] = {}
        for r in derived:
