"""0-shot MMLU through a running engine: the serving accuracy gate, and the
before/after gate for a training run — one letter per question, argmax over
the four letter ids, the lm-eval convention."""
...
def mmlu_score(engine: Any, tok: Any, prompts: list[str], concurrency: int = 32) -> list[str]:
    """One greedy letter per prompt. ``concurrency`` <= the engine's state
    slots: submit allocates one eagerly."""
