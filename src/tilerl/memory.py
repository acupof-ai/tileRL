...
def reserve_memory_fraction(total_bytes: int, reserve_bytes: int) -> float:
    """torch.cuda.set_per_process_memory_fraction argument that HOLDS reserve_bytes.

    Unlike the peak-live block trim, the fraction is a real allocator fence: cudaMalloc
    past it fails outright (catchable torch.cuda.OutOfMemoryError on sm70, verified by
    ~/probe_memory_fraction.py), so the bytes cannot be re-reserved by the caching
    allocator. Pure arithmetic; 0 reserve -> 1.0 (full card, exact no-op). Raises when
    the reserve meets or exceeds the card.
    """
    if reserve_bytes < 0:
        raise ValueError(f"device reserve must be >= 0, got {reserve_bytes}")
    if reserve_bytes >= total_bytes:
        raise ValueError(
            f"device reserve {reserve_bytes} bytes meets/exceeds total {total_bytes}")
    return (total_bytes - reserve_bytes) / total_bytes
