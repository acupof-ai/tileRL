...
def per_kv_block_bytes(cfg, kv_io, kv_fp8=None, draft_layers: int = 0) -> int:
    ...
    they cannot disagree. K+V over every full-attn plane (incl. the fp8 per-head_dim f32
    scale), plus the draft's plain-IO pool (no scale plane).
    """
...
def plan(cfg, params: dict | None, device_free: int, *, num_slots: int, num_blocks: int,
    ...
    rows.append(Row("device", "kv_pool",
                    ...
                    f"{num_blocks} blocks"))
    if draft_layers:
        draft_per_block = draft_layers * nbytes(
            _dtype_fmt(kv_io), (2, cfg.num_kv_heads, BLOCK_TOKENS, cfg.head_dim))
        rows.append(Row("device", "draft_pool", draft_per_block * num_blocks, "draft K+V"))
