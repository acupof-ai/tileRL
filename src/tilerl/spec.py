...
#: Default trailing READ window for draft decode attention, in tokens. 0 = read the
#: full prefix (the pre-window behavior); this is the production default until the
#: W-sweep's live post-deploy 32k confirmation lands, so serving does NOT silently
#: truncate context. 2048 is the device-measured candidate (accept-rate within
#: ~3.4 points of full prefix at 9k/16k/32k, equal-or-better vs 1024), opt-in via
#: --draft-attn-window-tokens / TILERL_DRAFT_ATTN_WINDOW_TOKENS.
#:
...
#: table. Set the window via the serve flag/env; 0 restores the full prefix.
DRAFT_ATTN_WINDOW_TOKENS_DEFAULT = 0
#: The device-selected candidate from the V100 W-sweep, kept as a named value so
#: callers/docs can opt into the measured window without it being the default.
DRAFT_ATTN_WINDOW_TOKENS_RECOMMENDED = 2048
...
class DraftHead:
    ...
    def __init__(self, trunk: Any, params: dict[str, torch.Tensor],
                 num_layers: int = 1, attn_window_tokens: int = DRAFT_ATTN_WINDOW_TOKENS_DEFAULT) -> None:
        from .model import Model
        ...
        # Trailing READ window for draft decode attention, in tokens. 0 (the
        # default) = the full prefix, the pre-window behavior; >0 truncates only
        # the attention READ view (the draft still WRITES and RETAINS its whole
        # dense KV). Resolved by the loader/CLI, not read from the environment
        # here, so a directly-built head is injectable and testable.
        self.attn_window_tokens = int(attn_window_tokens)
...
def resolve_draft_attn_window_tokens(explicit: int | None = None) -> int:
    """Resolve the draft trailing-READ window once, at the load boundary.

    An explicit caller value (the serve CLI flag) wins; otherwise the
    TILERL_DRAFT_ATTN_WINDOW_TOKENS env; otherwise the module default (0 = full
    prefix). The DraftHead itself takes a plain int and never reads the env, so
    direct construction is injectable and there is one place the env is honored.
    """
    if explicit is not None:
        return int(explicit)
    return int(os.environ.get(
        "TILERL_DRAFT_ATTN_WINDOW_TOKENS", str(DRAFT_ATTN_WINDOW_TOKENS_DEFAULT)))


def load_draft(trunk: Any, path: str | Path,
               attn_window_tokens: int | None = None) -> Any:
    ...
    ``attn_window_tokens`` applies to the chain DraftHead only (the block drafter
    projects its K/V from trunk aux taps and has no attention window); None leaves
    env→default resolution to :func:`resolve_draft_attn_window_tokens`."""
    from safetensors import safe_open
    ...
    window = resolve_draft_attn_window_tokens(attn_window_tokens)
    with safe_open(str(path), "pt", device="cpu") as f:
