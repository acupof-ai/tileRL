...
def train_step(
    ...
    def grad_fn(logits, chunk, lo):
        ...
        # CE averages over this chunk's rows; rescale to the batch's. In place:
        # the gradient is the [B,T,vocab] logit buffer itself.
...
def rl_step(
    ...
    pos = np.arange(t)
    # Scored positions over the WHOLE batch. Micro-batching must not move this
    # normalizer, or the gradient would depend on how the rows were chopped up.
    n = float(((pos >= plen - 1) & (pos < slen - 1)).sum())

    def grad_fn(logits, chunk, lo):
        bm, tm, _ = logits.shape
        loss, grad = backend.cross_entropy_loss_grad(logits, chunk)
        dev = grad.device
        ...
        # CE averages over bm*(tm-1) positions; rescale to the batch's scored
        # count so the step size does not depend on how much prompt or padding
        # is in the batch, nor on the micro-batch size.
