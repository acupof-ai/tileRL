"""The served pages: the landing page, the chat playground, and the markdown
renderer the playground ships to the browser.

Split out of server.py, where 750 of 1107 lines were these three strings. They are
assets, not logic -- kept as Python constants rather than package data files so
`tests/test_chat_ui.py` can keep importing and parsing them, which is how the XSS,
CSS-scoping and DOM-id gates work.
"""
