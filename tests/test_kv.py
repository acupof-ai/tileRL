...
#: (n, budget) pairs, including schedules where decode rows sharing the tick
#: shrink the budget and shift the whole chunk walk. The held-snapshot fix:
#: errors/2026-09-08-a-one-token-chunk-made-last-unreachable.md
_BOUNDARY_CASES = [
    (65, 512), (129, 512), (513, 512), (1025, 512),
    (33, 16), (18, 17), (49, 8), (962, 511), (961, 504),
]


@pytest.mark.parametrize("n,budget", _BOUNDARY_CASES)
def test_a_ragged_prompt_publishes_the_deepest_boundary_the_walk_reached(n, budget):
    """A ragged prompt must leave an entry at the deepest block-aligned chunk end its
    actual schedule reached -- which depends on the per-tick budget
    (`max_num_batched_tokens - len(decodes)`), not on n alone, so no predictor can
    name it. The engine holds that boundary's exact snapshot and inserts it at
    completion.

    The expected length comes from a planner-only drive: the schedule is the
    source of truth, and a fixed table here would re-introduce a predictor.
    Red on main for (33,16) and (49,8) -- a decode-sized budget shifts the walk
    so the n-only predictor misses the real deepest boundary and no entry lands
    ...
    cfg = tiny(max_position_embeddings=4096)
    toks = [3 + (i * 37 + 5) % 317 for i in range(n)]

    planner = build_engine(cfg, build_random(cfg, seed=5), RefBackend(), num_blocks=512,
                           num_slots=2, max_batch=1, max_total_tokens=4096,
                           max_num_batched_tokens=budget, sparse_k=0)
    planner.submit(toks, SamplingParams(max_new_tokens=2, seed=0))
    ends, at = [], 0
    while at < n:                          # drive the planner, the only source of chunk ends
        _, prefills, chunks = planner._build_plan()
    ...
    assert n % BLOCK_TOKENS, "case table is ragged prompts only"
    deepest = max(e for e in ends if e % BLOCK_TOKENS == 0)

    engine = build_engine(cfg, build_random(cfg, seed=5), RefBackend(), num_blocks=512,
                          num_slots=2, max_batch=1, max_total_tokens=4096,
                          max_num_batched_tokens=budget, sparse_k=0)
    rid = engine.submit(toks, SamplingParams(max_new_tokens=2, seed=0))
    for _ in range(2000):
        engine.step()
        if rid in engine.poll():
            break
    else:
        raise AssertionError(f"n={n} budget={budget} never finished")

    lengths = {len(e.tokens) for e in engine._prefix._by_id.values()}
    assert deepest in lengths, (
        f"n={n} budget={budget}: walk ends at {ends}, deepest aligned is {deepest}, "
        f"store holds {sorted(lengths)} -- the last boundary never published, so a "
        "second turn sharing that prefix re-prefills it")
