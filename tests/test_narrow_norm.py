...
def test_narrow_is_a_no_op_where_the_gemv_takes_f32():
    ...
    assert 'key = "rmsnorm_apply_narrow" if narrow and self.gemv_io != torch.float32' in src, (
        "backend.rmsnorm must gate the narrow kernel on gemv_io, not on `narrow` alone"
    )
