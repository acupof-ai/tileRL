...
def test_paged_attention_vs_naive(backend):
    ...
    k1, v1 = k_cache[:, :1].contiguous(), v_cache[:, :1].contiguous()
    for w, n in ((4, 24), (8, 28)):
        qw = torch.randn(b, w, 8, d)
        lens_w = torch.tensor([n, n - 8], dtype=torch.int32)
        _assert_close(
            backend.paged_attention(qw, k1, v1, block_table, lens_w, scale),
            _naive_paged(qw, k1, v1, block_table, lens_w, scale),
            f"paged_attention verify width {w}",
        )
