...
def test_v1_messages_never_answers_200_for_an_engine_failure(tmp_path, monkeypatch):
    ...
    class _Dies:
        ...
        Keying on the RESULT instead of on a call count removes the race: a `take` that
        returns None means the request is still running and the route keeps waiting, which
        is not the moment under test; the first one carrying a result is.
        """
